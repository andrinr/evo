...
/// * `velocity_control` - Desired velocity from brain output (controls velocity directly)
...
pub fn execute_movement(
    entity: &mut Organism,
    velocity_control: f32,
    params: &Params,
    dt: f32,
) -> ActionResult {
    // Calculate desired acceleration based on brain output
    let acceleration = -Array1::from_vec(vec![
        velocity_control * entity.rot.cos(),
        velocity_control * entity.rot.sin(),
    ]) * params.move_multiplier;

    // Update velocity
    // entity.vel = entity.vel.clone() + acceleration * dt;
    entity.vel = acceleration;

    // Energy consumption is proportional to acceleration (force needed to change velocity)
    entity.consume_energy(velocity_control * dt * params.move_energy_rate);

    vec![]
}

/// Executes movement rotation action for an organism.
///
/// # Arguments
///
...
pub fn execute_rotation(
    ...
    entity.rot += rotation * dt * params.rot_multiplier;
    entity.consume_energy(rotation.abs() * dt * params.rot_energy_rate);

    vec![]
}

/// Executes vision rotation action for an organism.
///
/// Vision rotation is now relative to body rotation - the neural network
/// controls the offset angle between where the organism looks and where it moves.
///
/// # Arguments
///
/// * `entity` - The organism to rotate vision
/// * `vision_rotation_offset` - Desired vision offset from body rotation (from brain output)
/// * `params` - Simulation parameters
/// * `dt` - Time delta
///
/// # Returns
///
/// Empty vector (no events generated by vision rotation).
pub fn execute_vision_rotation(
    entity: &mut Organism,
    vision_rotation_offset: f32,
    params: &Params,
    dt: f32,
) -> ActionResult {
    // Set vision_rot to be relative to body rotation
    // The brain output directly controls the offset angle
    entity.vision_rot += dt * params.rot_multiplier * vision_rotation_offset;

    // No energy cost - the offset is just a control signal, not actual rotation work
    vec![]
}
...
pub fn execute_all_actions(
    ...
    let movement_rotation = brain_outputs[offset];
    let vision_rotation = brain_outputs[offset + 1];
    let velocity = brain_outputs[offset + 2];
    let attack_strength = brain_outputs[offset + 3];
    let share_amount = brain_outputs[offset + 4];
    // Reproduction outputs removed - all reproduction now happens via ecosystem.spawn()

    let mut events = vec![];

    // Execute all actions
    events.extend(execute_rotation(entity, movement_rotation, params, dt));
    events.extend(execute_vision_rotation(entity, vision_rotation, params, dt));
