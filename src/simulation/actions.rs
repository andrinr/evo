...
pub fn execute_movement(
    ...
    // Calculate desired velocity based on brain output
    let desired_vel = Array1::from_vec(vec![
        velocity_control * entity.rot.cos(),
        velocity_control * entity.rot.sin(),
    ]) * params.move_multiplier;

    // Calculate acceleration (change in velocity)
    let acceleration = (&desired_vel - &entity.vel) / dt;
    let acceleration_magnitude = acceleration.mapv(|x| x * x).sum().sqrt();

    // Update velocity
    entity.vel = desired_vel;

    // Energy consumption is proportional to acceleration (force needed to change velocity)
    entity.consume_energy(acceleration_magnitude * dt * params.move_energy_rate);
